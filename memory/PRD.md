...
- ✅ **NEW Jun 2026 — Cinetekk-inspired redesign, LIGHT/RED theme (Phase 1)** — Per user pivot ("change background to white and red like cinetekk"). Kept editorial typography (Plus Jakarta Sans + Inter + Playfair). `index.css` now defines a WHITE background + RED (#D62828) accent theme via a site-wide global palette remap that overrides all existing hardcoded dark-theme utility classes: dark surfaces→white/off-white, light text→dark, cyan/gold accents→red, cyan gradient CTAs→solid red w/ white text, borders→black hairlines, and dark hero image-overlay gradients→light scrims (so dark hero text stays readable). `Header.jsx` & `Hero.jsx` rebuilt for light theme (dark logo via brightness-0, red CTAs, framed cinematic image w/ review card). `Footer.jsx` logo→dark. `BlogArticle.jsx` prose switched from `prose-invert` to light config (dark text, red H2/links, light tables) — fixed invisible bold text. Fixed gradient-clipped cyan headings (from-cyan-400/to-sky-400→red) and translucent form inputs (bg-gray-800/50→light). VERIFIED on homepage, /services/home-theatre, blog article, /contact. Content & SEO keywords 100% unchanged. REMAINING: spot-check/polish packages, process, projects, blog-list, 27 location pages, 5 brand pages, ad landing; and (optional) restyle section LAYOUTS to fuller editorial (bento/asymmetric) per design_guidelines.json.
